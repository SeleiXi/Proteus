...
the host npm Codex installation. Only the boundary compiler runs as container root so it
can update the root-owned cache; model-driven phases keep the host uid/gid. Runtime model
access uses a read-only bind of the user's
